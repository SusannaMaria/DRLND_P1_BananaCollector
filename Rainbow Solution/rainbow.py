...
class DQNAgent:
    ...
    def __init__(
        ...
        print(f"Brain Name: {self.brain_name}")

        # reset the environment
        env_info = env.reset(train_mode=True)[self.brain_name]
        state = env_info.vector_observations[0]            # get the current state

        self.banana_action_size = self.brain.vector_action_space_size
        self.banana_state_size = len(env_info.vector_observations[0])
        
        obs_dim = self.banana_state_size
        action_dim = self.banana_action_size
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, np.float64, bool]:
        ...
        #print(action.item(0))
        env_info = self.env.step(action.item(0))[self.brain_name]
    ...
    def train(self, num_frames: int, plotting_interval: int = 200, num_episodes: int = 1000):
        ...
        for frame_idx in range(1, num_frames + 1):        
            ...
            if done:
                ...
                if n_episodes % 10 == 0:
                    print('\rEpisode {}\tFrame: {}\tScore: {}'.format(n_episodes, frame_idx, np.mean(scores_10)))
                scores_10.clear()
                score =0
                n_episodes += 1 

                if n_episodes % 100 == 0:
                    self.save('checkpoint_rainbow_{:06d}.pth'.format(n_episodes))                
